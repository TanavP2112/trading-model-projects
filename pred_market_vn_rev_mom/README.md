...
This asymptotic normality is preserved by the robust Huber-White "sandwich" covariance matrix. However, this property still requires the underlying process to be strictly stationary ($$E[\ln(\alpha z_t^2 + \beta)] < 0$$), meaning we cannot arbitrarily ignore the parameter constraints. QMLE is leveraged specifically because it yields asymptotically valid standard errors and consistent point estimates despite the non-Gaussianity forced by the \([0,1]\) boundaries. This is especially vital for the joint `GARCH+DR-AS` model, where QMLE isolates the generic clustering parameters from the structural Wright-Fisher (`DR`) and adverse-selection (`ν(V)`) components without needing to map the complex joint distribution of the error terms.

**Quick Note About Persistence** `(volatility_model.py)`: QMLE consistency of $$\hat\theta_M$$ says nothing about whether the fitted process is well-behaved, since that's a separate question about the parameters themselves. We therefore store $$\alpha+\beta$$ (persistence) alongside each model. The asymptotic normality result still presupposes $$E[\ln(\alpha z_t^2+\beta)]<0$$, so a fitted persistence near our optimizer's boundary ($$\alpha+\beta<0.98$$) signals a near-integrated process the model may be misspecifying — something QMLE's distributional robustness doesn't fix.
