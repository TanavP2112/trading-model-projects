...
def run_expanding_walk_forward(
    ...
    bankroll = 100_000.0  # matches backtest.BANKROLL
    ann = 365.0
    if "category" in full_wf_trades.columns and not full_wf_trades.empty:
        ...
        def _pooled_sharpe(g: pd.DataFrame) -> float:
            daily = g.groupby("exit_date")["net_pnl"].sum() / bankroll
            if len(daily) < 2 or daily.std(ddof=1) == 0:
                return np.nan
            return float(daily.mean() / daily.std(ddof=1) * np.sqrt(ann))

        def _pooled_n_days(g: pd.DataFrame) -> int:
            return int(g["exit_date"].nunique())

        def _fold_sharpe_stats(g: pd.DataFrame) -> pd.Series:
            # Sharpe within each eval period, then stats across those periods.
            fold_sharpes = []
            for _, fg in g.groupby("eval_period"):
                daily = fg.groupby("exit_date")["net_pnl"].sum() / bankroll
                if len(daily) < 2 or daily.std(ddof=1) == 0:
                    continue
                fold_sharpes.append(daily.mean() / daily.std(ddof=1) * np.sqrt(ann))
            if not fold_sharpes:
                return pd.Series({"mean_fold_sharpe": np.nan, "std_fold_sharpe": np.nan, "n_folds_with_sharpe": 0})
        ...
        for (strat, H, cat), g in tr.groupby(["strategy", "horizon_hours", "category"]):
            ...
            n_days = _pooled_n_days(g)
            fs = _fold_sharpe_stats(g)
            # Volatility, max drawdown, turnover -- computed on POOLED daily
            # P&L for this (strategy, horizon, category), so numbers describe
            # the aggregate strategy rather than any one fold.
            daily_pnl_cat = g.groupby("exit_date")["net_pnl"].sum().sort_index()
            daily_ret_cat = daily_pnl_cat / bankroll
            if len(daily_ret_cat) >= 2:
                daily_vol_val = float(daily_ret_cat.std(ddof=1))
            else:
                daily_vol_val = float("nan")
            equity = bankroll + daily_pnl_cat.cumsum()
            running_max = equity.cummax()
            drawdown = (equity - running_max) / running_max
            max_dd_val = float(drawdown.min()) if len(drawdown) else float("nan")
            turnover_val = float(g["shares"].sum() / max(n_days, 1)) if n_days > 0 else float("nan")
